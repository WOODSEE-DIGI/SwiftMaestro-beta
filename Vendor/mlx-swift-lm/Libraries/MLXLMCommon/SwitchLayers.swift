...
public func gatherSort(x: MLXArray, indices: MLXArray) -> (MLXArray, MLXArray, MLXArray) {
    ...
    // Use MLX.take instead of subscript to avoid expandEllipsisOperations crashes
    // on certain tensor shapes (e.g. Qwen3MoE 4-bit with 128 experts).
    return (
        MLX.take(x.flattened(start: 0, end: -3), order.floorDivide(m), axis: 0),
        MLX.take(indices, order, axis: 0),
        inverseOrder
    )
}
...
public func scatterUnsort(x: MLXArray, invOrder: MLXArray, shape: [Int]? = nil) -> MLXArray {
    // Use MLX.take instead of subscript to avoid expandEllipsisOperations crashes.
    var x = MLX.take(x, invOrder, axis: 0)
    if let shape {

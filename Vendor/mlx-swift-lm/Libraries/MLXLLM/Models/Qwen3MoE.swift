...
class Qwen3MoESparseMoeBlock: Module, UnaryLayer {
    ...
    func callAsFunction(_ x: MLXArray) -> MLXArray {
        // Match upstream Python qwen3_moe.py:
        //   gates = mx.softmax(self.gate(x), axis=-1, precise=True)
        //   inds = mx.argpartition(gates, kth=-k, axis=-1)[..., -k:]
        //   scores = mx.take_along_axis(gates, inds, axis=-1)
        //   y = (y * scores[..., None]).sum(axis=-2)
        //
        // NOTE: We use MLX.take instead of the [..., (kth)...] subscript
        // because mlx-swift's expandEllipsisOperations crashes on nested
        // subscript calls for certain tensor shapes (Qwen3MoE 4-bit with
        // 128 experts). MLX.take is a C function that avoids this path.
        var gates = gate(x)
        gates = MLX.softmax(gates, axis: -1, precise: true)

        let k = topK
        let kth = gates.dim(-1) - k
        let partitioned = MLX.argPartition(gates, kth: kth, axis: -1)
        let lastKIndices = MLXArray(Array(kth ..< gates.dim(-1)))
        let inds = MLX.take(partitioned, lastKIndices, axis: -1)

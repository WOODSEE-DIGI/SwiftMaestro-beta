...
public extension AudioProcessor {
    ...
    func assignAudioInput(inputNode: AVAudioInputNode, inputDeviceID: AudioDeviceID) {
        ...
        guard inputDeviceID != 0 else {
            Logging.error("Refusing to assign input device ID 0 (system object).")
            return
        }

        // Verify the requested device still exists before asking the AUHAL to use it.
        var propertyAddress = AudioObjectPropertyAddress(
        ...
        let status = AudioObjectGetPropertyData(inputDeviceID, &propertyAddress, 0, nil, &size, &name)
        guard status == noErr else {
            Logging.error("Input device ID \(inputDeviceID) is not valid (status \(status)); keeping default input device.")
            return
        }
    ...
    func setupEngine(inputDeviceID: DeviceID? = nil) throws -> AVAudioEngine {
        ...
        // Defensive checks: an invalid device or a misconfigured audio session can produce
        // a zero sample rate or zero channel count. Passing that to installTap raises an
        // uncaught exception: IsFormatSampleRateAndChannelCountValid(format).
        guard hardwareSampleRate > 0, inputFormat.channelCount > 0 else {
            throw WhisperError.audioProcessingFailed("Invalid input format: sample rate \(hardwareSampleRate), channels \(inputFormat.channelCount). Check microphone access and device selection.")
        }
